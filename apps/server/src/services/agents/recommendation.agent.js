...
class RecommendationAgent {
     ...
     */
    static async recommend(input, memoryContext, model) {
        ...
        if (memoryContext.allergies.length > 0) {
            filteredItems = filteredItems.filter(item => {
                return !item.allergens.some(allergen => memoryContext.allergies.some(a => a.toLowerCase() === allergen.toLowerCase()));
            });
        }
        ...
        if (isVegan) {
            filteredItems = filteredItems.filter(item => item.tags.some(t => t.toLowerCase() === 'vegan'));
        }
        else if (isVeg) {
            filteredItems = filteredItems.filter(item => item.tags.some(t => t.toLowerCase() === 'vegetarian'));
        }
        // Spice level filter
...
{{
  ...
  "recommendedIds": ["id1", "id2"]
                ...
                const selectedIds = parsed.recommendedIds || [];
