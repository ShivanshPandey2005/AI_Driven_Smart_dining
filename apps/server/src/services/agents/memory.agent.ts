...
export interface MemoryContext {
  preferences: string[]; // ['vegetarian', 'spicy', 'prefers desserts']
...
export class ContextMemoryAgent {
   ...
   */
  public static async getContext(sessionId: string): Promise<MemoryContext> {
    try {
   ...
   */
  public static async extractAndStore(
    sessionId: string,
    message: string,
    ...
    let extracted: Partial<MemoryContext> = {};
...
Response format MUST be raw JSON:
{
  ...
  private static fallbackExtraction(message: string, current: MemoryContext): Partial<MemoryContext> {
    const lower = message.toLowerCase();
    const extracted: Partial<MemoryContext> = {
      preferences: [],
      allergies: [],
      budget: 'unspecified',
      spiceTolerance: 'unspecified',
