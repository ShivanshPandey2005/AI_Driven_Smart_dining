...
export interface NluResult {
  ...
  translatedInput: string; // The normalized input in English for internal agent processing
...
Response format MUST be raw JSON:
{
  ...
  private static fallbackNlu(input: string): NluResult {
    ...
    let detectedLanguage: 'English' | 'Hinglish' | 'Telugu-English' | 'Other' = 'English';
   ...
   */
  public static async localizeResponse(
    response: string,
    targetLang: 'English' | 'Hinglish' | 'Telugu-English' | 'Other',
    model: ChatOpenAI | null
  ): Promise<string> {
